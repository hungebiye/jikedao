<template>
  <div id="orderDetail">
    ...
    <div class="allCard2">
      <div v-for="(item, index) in $store.state.user.userAddressList" :key="index + 'address'">
        <el-card class="personalAddress" :class="active == index ? 'active2' : ''">
          <div @click="show(index)">
            <div style="font-weight: 700">{{ item.receiver }}</div>
            <span>所在区域：</span>
            {{ item.userAddress }}
            <span>详细地址：</span>
            {{ item.userAddressDetails }}
            <span>联系电话:</span>
            {{ item.userPhone }}
          </div>
        </el-card>
    ...
    <commodity-list v-if="isCar" :isCar2="true" :commodityList="$store.state.Shopping_Car" :componentName="'购物车'"></commodity-list>
    <commodity-list :changeNumber="true" v-else :commodityList="commodityList"></commodity-list>
    <el-button v-if="isCar" @click="Submit_Orders($store.state.Shopping_Car, receivingAddress[active])">
      确认购买
    </el-button>
    <el-button v-else @click="Submit_Orders_fromShop(commodityList[0], receivingAddress[active])">
      确认购买
    </el-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    Submit_Orders(commodity, receivingAddress) {
      ...
      let createdTime = `${date.getFullYear()}年${date.getMonth() + 1}月${date.getDate()}日${date.getHours()}:${date.getMinutes()}`
