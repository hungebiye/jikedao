  ...
  <div id="cm-details">
    <div class="cm-detalis">
      <div class="cm-img">
        ...
        <img style="width: 80%; height: 80%; margin-top: 10%" :src="commodityData.img" />
      </div>
      <div class="cm-shop">
        ...
        <div class="color">
          ...
          <br />
          <div class="flex">
            <el-radio v-model="price" label="1" border>￥{{ Math.floor(commodityData.price / 3) }} x 3期</el-radio>
            <el-radio v-model="price" label="2" border>￥{{ Math.floor(commodityData.price / 6) }} x 6期</el-radio>
            <el-radio v-model="price" label="3" border>￥{{ Math.floor(commodityData.price / 12) }} x 12期</el-radio>
            <el-radio v-model="price" label="4" border>￥{{ Math.floor(commodityData.price / 24) }} x 24期</el-radio>
          </div>
        </div>
        <el-button @click="commodityData.number++">+</el-button>
        {{ commodityData.number == 0 ? (commodityData.number = 1) : commodityData.number }}
        <el-button @click="commodityData.number > 0 ? commodityData.number-- : ''">-</el-button>
        <div class="btn">
          <el-button
            type="primary"
            :disabled="commodityData.isSeckill ? status !== '结束' : false"
            @click="pushCar(commodityData, commodityData.id, commodityData.name, commodityData.img, commodityData.price, commodityData.number)"
          >
            加入购物车
          </el-button>
          <el-button type="primary" :disabled="commodityData.isSeckill ? status !== '结束' : false" @click="Submit_Orders_fromShop(commodityData)">
            立即购买
          </el-button>
        </div>
      </div>
