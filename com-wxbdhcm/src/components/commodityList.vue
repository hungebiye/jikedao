      ...
      <el-col :span="22">
        <div class="grid-content bg-purple">
          <div class="top" v-if="route">
            <el-button type="primary" @click="$router.push('/Shopping')">商品列表</el-button>
            <span
              :style="{
                'line-height': '50px',
                'list-style': 'none',
            ...
            <el-button type="primary" @click="$router.push(route[0].path)">{{ route[0].name }}</el-button>
          </div>

          <el-table :data="commodityList" border style="width: 100%" max-height="530">
            ...
            <el-table-column prop="number" label="数量">
              <template slot-scope="scope">
                ...
                {{ scope.row.number }}
                <el-button @click="if (scope.row.number > 1) scope.row.number--" v-if="changeNumber">-</el-button>
              </template>
            </el-table-column>
          ...
          <div class="bottom-div" v-if="isCar">
            <el-button type="primary" :style="{ 'line-height': '22px', 'text-align': 'center' }" v-on:click="Submit_Orders">
              提交订单
            </el-button>
          </div>
        </div>
